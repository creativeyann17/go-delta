// Package fastcdc is vendored from github.com/jotfs/fastcdc-go v0.2.0
// (MIT license, see LICENSE in this directory).
//
// Local fix vs upstream: NewChunker used to XOR the package-level gear table
// with opts.Seed in place, racing with concurrent NewChunker/Next calls and
// corrupting the table for non-zero seeds. Each Chunker now carries its own
// seeded copy of the table.
package fastcdc
...
type Chunker struct {
	...
	table [256]uint64 // per-chunker seeded copy of the gear table
...
func NewChunker(rd io.Reader, opts Options) (*Chunker, error) {
	...
	// Seed a private copy of the gear table (upstream mutated the global)
	for i := range table {
		chunker.table[i] = table[i] ^ opts.Seed
	}
